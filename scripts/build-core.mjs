 ...
 *   2. Generates src/cards/core-index.ts  (root barrel — all core cards).
 *   3. Runs `vite build --config vite.lib.config.ts`.
...
// ---------------------------------------------------------------------------
// Step 1 — Generate src/cards/core-index.ts
// ---------------------------------------------------------------------------

const CORE_INDEX_PATH = join(ROOT, "src", "cards", "core-index.ts");

const coreIndexLines = [
  "// AUTO-GENERATED by scripts/build-core.mjs — do not edit manually.",
  "// Importing this module registers all @pihanga2/shadcn core cards with the",
  "// Pihanga runtime via their side-effectful index.ts registerCardComponent() calls.",
  "",
  ...CORE_CARDS.map((card) => `export * from "./${card}/index.js";`),
  "",
];

const coreIndexContent = coreIndexLines.join("\n");

console.log(
  `\n📝  Generating src/cards/core-index.ts  (${CORE_CARDS.length} cards)…`,
);
if (!DRY_RUN) {
  writeFileSync(CORE_INDEX_PATH, coreIndexContent, "utf-8");
  console.log("    ✓  Written");
} else {
...
// ---------------------------------------------------------------------------
// Step 3 — Aggregate dependencies from core card dependencies.json files
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Step 4 — Build exports map
// ---------------------------------------------------------------------------

const exportsMap = {
  // Root barrel — registers all core cards at once
  ".": {
...
// ---------------------------------------------------------------------------
// Step 6 — Write dist-lib/README.md
// ---------------------------------------------------------------------------
